...
//tag barcode specific counts in windows around specified positions

SEXP cwindow_n_tags(SEXP pos_R, SEXP barcode_R, SEXP wpos_R, SEXP window_half_size_R, SEXP cmatrix_R) {
  double* pos=REAL(pos_R);
  int* bcode=INTEGER(barcode_R);
  int n=LENGTH(pos_R);
  double* wpos=REAL(wpos_R);
  int nw=LENGTH(wpos_R); // number of windows
  double whs=(double) *INTEGER(window_half_size_R);
  int* count=INTEGER(cmatrix_R)
