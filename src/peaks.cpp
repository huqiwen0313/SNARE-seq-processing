...
extern "C" {
  ...
  // return: vector of relative coordinates (x), indecies of point positions (xi), and indecies of matched data points (xj)
  // was calculated (i).
				...
				SEXP size_R)
  {
    ...
    vector<int> x; vector<int> xi; vector<int> xj;
    ...
    i=0;
    for(vector<int> ::const_iterator pi=xj.begin();pi!=xj.end();++pi) {
      xjv[i++]=1+(*pi);
    }
  ...
  // note: all vectors, except for fi_R are doubles
  ...
  SEXP find_poisson_enrichment_clusters(SEXP pos_R,SEXP flag_R,SEXP wsize_R,SEXP thr_R,SEXP mcs_R,SEXP bgm_R,SEXP mintag_R) {
...
#ifdef DEBUG  
    Rprintf("nt=%d; wsize=%d; thr=%f; mcs=%d; min.tag=%f\n",nt,wsize,thr,mcs,mintag);
#endif
	...
	// calculate z score
	double z=0;
	if(((double)cc[1]) > bgm*cc[0]) {
	  double dv=sqrt(((double)(cc[0]*bgm + cc[1])/2));
	  if(dv>0) {
	    double en=(fabs((double) (cc[0]*bgm - cc[1])));
	    z=en/dv;
	  }
	}
...
#ifdef DEBUG  
	Rprintf("s=%d (%f); e=%d (%f); window: %f-%f; cc=[%d,%d]; z=%f\n",si,pos[si],ei,pos[ei],ws,ws+wsize,cc[0],cc[1],z);
#endif
      
	if(z>=thr && cc[1]>=mintag) {
	  if(inclust) {
